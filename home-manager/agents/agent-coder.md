...
- **Source Control**:
  Create atomic commits that encapsulate a cohesive,
  incremental,
  logical change.
  Don't be too granular with commits—include smaller prerequisites in the same change.
  Commits should focus on the task at hand.
  Keep commit messages brief,
  unless the issue is complex.
  Do not add a detailed description if the change is obvious and the summary already explains enough.
